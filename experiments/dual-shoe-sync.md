...
right now the system is one-shoe. each shoe is independent. dual-shoe sync means: a tap on the left heel changes both shoes; the wearer can also pair with another wearer's shoe and "handshake" emotionally — both shoes pulsing a shared colour briefly.

two distinct features sit under this umbrella:

1. **paired left/right (single wearer)** — both shoes always show the same emotional state. solves the asymmetry problem of one-shoe-only wear.
...
it's also the feature that makes the project legibly a *product* rather than a one-off prototype. paired wearables are a coherent product category in a way that lone wearables are not.
...
this is a deliberate pairing — not always-on. the social contract is "we both opted into this expression."
...
- new file: `firmware/main/sync.h`
  ...
  - `syncUpdate()` — process incoming peer state, returns events to main.ino
...
- **failure mode**: what does a shoe do when its peer is offline? naive answer: act as a single shoe. but if a wearer is used to seeing their shoes mirror, asymmetric behaviour reads as "broken." needs a small "peer disconnected" indication (subtle desaturation? slower transitions?).
...
4. **does pairing also share private/public state?** subtle: if I'm in private and my shoe pairs with yours, do you become private too? recommend no — each wearer keeps their own privacy setting.
5. **time-sync strategy?** (a) periodic broadcast every 60s, (b) shared epoch on connection, (c) ignore drift.

## one-paragraph elevator pitch

> two shoes that share emotional state. on a single wearer, one tap changes both. between two wearers who choose to pair, the shoes can briefly express together — a 30-second shared rainbow, then back to their own emotional baselines. it turns a personal wearable into a relational one without making either pole feel less honest.
