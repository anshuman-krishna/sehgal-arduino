...
> the shoe responds to how the wearer is moving — not just what they tap.

## what it adds

the system today knows about taps and time. it doesn't know whether the wearer is sitting, standing, walking, or running. motion-reactive behaviour adds a small accelerometer and uses its data to *modulate* (not replace) the existing emotional output.
...
2. **walking taper** — during active walking, the brightness softens by ~30% (so the shoe doesn't "shout" while the wearer is in motion). also nudges happy and cool toward calmer timings.
...
right now the shoe is reactive only to taps. that means it's a "command and execute" relationship. motion-reactive makes the shoe *aware* — like the difference between a remote-control toy and a cat.

a shoe that gets brighter when you sit still is the kind of small detail that makes the wearable feel alive without ever being pushy. a shoe that responds to a jump with a flash of celebration is genuinely playful. these are emotional capabilities the current system simply cannot produce.
...
it's not a new mode — it's an overlay. the shoe is still in (say) calm; it just briefly broke into a smile. think of it as emotional punctuation.
...
- **stillness vs sitting still vs sleeping**: should a wearer sleeping in their shoes get the stillness boost? probably not — that's where the existing idle dim-down should take over. need to make sure the two systems don't fight each other (idle dimming while stillness boosting is contradictory).
- **impact threshold tuning**: 2.5g is a starting guess. some wearers walk heavy, some land softly when jumping. needs the same kind of threshold-tuning workflow as the piezo did in phase 2 — maybe a `motion_debug.ino` sketch.
...
2. **does the celebration animation pass through during walking?** small philosophical question. recommend yes — a deliberate jump while walking is a meaningful moment, not noise.
3. **stillness boost cap**: 1.2x is conservative. test in real wear before raising.
4. **does walking taper apply in private mode?** private already dims by 50%. additional walking dampening would push the shoe past invisible. recommend: private mode disables walking taper (private is already quiet enough).
...
> the shoe gets quieter when the wearer walks, brighter when they're still, and breaks into a small celebration when they jump. it turns a tap-driven wearable into one that's aware of context — present in a way the current system cannot be — without ever overriding the wearer's chosen emotional mode.
